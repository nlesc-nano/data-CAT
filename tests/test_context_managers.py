...
def test_openyaml() -> None:
    ...
    manager = MetaManager(path, OpenYaml)
    ...
    with manager.open(write=False) as s:
        assert_eq(s, ref)
...
def test_openlig() -> None:
    ...
    manager = MetaManager(path, OpenLig)
    ...
    with manager.open(write=False) as df:
        for k in ref:
...
def test_openqd() -> None:
    ...
    manager = MetaManager(path, OpenQD)
