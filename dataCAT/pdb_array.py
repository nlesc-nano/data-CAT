...
from typing import List, Tuple, Collection, NamedTuple, Iterable, Union, Type, TypeVar, Optional, overload, Generator, Sequence

import numpy as np
import pandas as pd
from scm.plams import Molecule, Atom, Bond
from nanoutils import TypedDict, SupportsIndex
...
_DTYPE_ATOM: _DType = [
    (i, dtype, shape or ()) for i, (dtype, shape) in DTYPE_ATOM.iterrows()
]
...
_DTYPE_BOND: _DType = [
    (i, dtype, shape or ()) for i, (dtype, shape) in DTYPE_BOND.iterrows()
]
...
_3Tup = Tuple[_Properties, Tuple[float, float, float], str]


def _iter_rec(atom_array: np.recarray) -> Generator[_3Tup, None, None]:
    for IsHeteroAtom, SerialNumber, Name, ResidueName, ChainId, ResidueNumber, coords, Occupancy, TempFactor, symbol, charge, charge_float in atom_array:  # noqa: E501
        ...
        _coords: Tuple[float, float, float] = tuple(coords)  # type: ignore
        yield properties, _coords, symbol.decode()


def rec_to_mol(atom_array: np.recarray, bond_array: np.recarray,
               atom_len: Optional[int] = None,
               bond_len: Optional[int] = None,
               mol: Optional[Molecule] = None) -> Molecule:
    """Construct a molecule from the passed **atom_array** and **bond_array**.
    """
    if mol is None:
        ret = Molecule()
...
class PDBTuple(NamedTuple):
    """A :func:`~collections.namedtuple` for holding an array-like represention of a set of .pdb files.
    ...
    .. code:: python
        ...
        >>> print(pdb_tup)  # doctest: +SKIP
        PDBTuple(
            atoms      = numpy.recarray(..., shape=(23, 2099), dtype=...),
            bonds      = numpy.recarray(..., shape=(23, 1950), dtype=...),
    ...
    bonds : :class:`numpy.recarray`, shape :math:`(n, k)`
        ...
        all field names, dtypes and shapes.
    atom_count : :class:`numpy.ndarray[int]<numpy.ndarray>`, shape :math:`(n,)`
        An ndarray for keeping track of the number of atoms in each molecule in :attr:`atoms`.
    bond_count : :class:`numpy.ndarray[int]<numpy.ndarray>`, shape :math:`(n,)`
        An ndarray for keeping track of the number of bonds in each molecule in :attr:`bonds`.
    ...
    def to_molecules(self, idx: SupportsIndex = ..., mol: Optional[Molecule] = ...) -> Molecule:
        ...
    def to_molecules(self, idx=None, mol=None):
        ...
        .. code:: python
            ...
            >>> pdb_tup.as_molecules(idx=0)
            <scm.plams.mol.molecule.Molecule at ...>

            # Create three new molecules from `pdb_tup`
            >>> pdb_tup.as_molecules(idx=[0, 1])
            [<scm.plams.mol.molecule.Molecule at ...>,
             <scm.plams.mol.molecule.Molecule at ...>]

            # Update `mol` with the info from `pdb_tup`
            >>> mol1 = Molecule(...)
            >>> mol2 = pdb_tup.as_molecules(idx=2, mol=mol1)
            >>> mol1 is mol2
            True

            # Update all molecules in `mol_list` with info from `pdb_tup`
            >>> mol_list1 = [Molecule(...), Molecule(...), Molecule(...)]
            >>> mol_list2 = pdb_tup.as_molecules(idx=range(3), mol=mol_list)
            >>> all((m1 is m2) for m1, m2 in zip(mol_list1, mol_list2))
            True
        ...
        mol : :class:`~scm.plams.mol.molecule.Molecule`, :class:`Iterable[Molecule]<typing.Iterable>`, optional
            A molecule or list of molecules.
            If one or molecules are provided here then they will be updated inplace.

        Returns
        -------
        :class:`~scm.plams.mol.molecule.Molecule` or :class:`List[Molecule]<typing.Molecule>`
            A molecule or list of molecules,
        ...
        if not is_seq:
            return rec_to_mol(atoms, bonds, atom_count, bond_count, mol)
        ...
        return [rec_to_mol(*args) for args in iterator]
