...
#: A mapping representing the dtype of the array returned by :func:`sanitize_datetime`.
DTYPE_DT: Mapping[str, np.dtype] = MappingProxyType({
    k: np.dtype(v) for k, v in _DTYPE_DT.items()
})
del _DTYPE_DT
_DTYPE_DT2 = np.dtype(list(DTYPE_DT.items()))


def sanitize_datetime(dt: datetime, **kwargs: Any) -> np.recarray:
    r"""Convert the passed **dt** object into a h5py-compatible numpy record array.
    ...
    dt : :class:`~datetime.datetime`
        A datetime object.
    \**kwargs : :data:`~typing.Any`
        Further keyword arguments for :func:`numpy.rec.array`.

    Returns
    -------
    :class:`numpy.recarray`
        A recarray containg **dt**.

    See Also
    --------
    :data:`DTYPE_DT`
        A Mapping representing the dtype of the to-be returned recarray.

    """
    dt_tup = tuple(getattr(dt, key) for key in DTYPE_DT.keys())
    return np.rec.array(dt_tup, dtype=_DTYPE_DT2, **kwargs)
...
#: A mapping representing the dtype of the array returned by :func:`sanitize_version`.
DTYPE_VERSION: Mapping[str, np.dtype] = MappingProxyType({
    k: np.dtype((v, 3)) for k, v in _DTYPE_VERSION.items()
})
del _DTYPE_VERSION
_DTYPE_VERSION2 = np.dtype(list(DTYPE_VERSION.items()))


def sanitize_version(*versions) -> np.recarray:
    r"""Convert the passed versions into a h5py-compatible numpy record array.

    Parameters
    ----------
    \*versions : :class:`Tuple[int, int, int]`
        3-tuples representing a set of :math:`n` versions.

    Returns
    -------
    :class:`numpy.recarray`, shape :math:`(n,)`
        A recarray containg the passed versions.
    ...
    """
    return np.rec.array(versions, dtype=_DTYPE_VERSION2)
